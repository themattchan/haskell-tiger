...
unEx (Cx genstm) = do
  ...
  f <- Temp.newLabel
  return $
    IR.ESeq (IR.seq [ IR.Move (IR.Temp r) (IR.Const 1)
                    , genstm t f
                    , IR.Label f
...
unNx (Nx s) = return s
unNx cx     = IR.Exp <$> unEx cx

unCx :: (Gensym Temp.Temp m) => Exp -> m (Temp.Label -> Temp.Label -> IR.Stm)
unCx = error "TODO"
-- unCx (Ex (IR.Const 0)) = return $ \_ f -> IR.Jump (IR.Name f [f])
-- unCx (Ex (IR.Const 1)) = return $ \t _ -> IR.Jump (IR.Name t [t])
